...
一个有缓存的通道相当于维护了三个个先进先出（FIFO）的队列
...
它的值如果为false就说明通道已经关闭，并且再没有元素值可取了，所以此方法判断通道是否关闭有一定的延迟性。
...
通道的长度代表通道缓冲队列当前包含的元素个数，容量就是初始化时你设置的那个数。

底层是双向链表

select中屏蔽已关闭的通道，可将通道置为nil，屏蔽该case

内部维护了三个队列，发送等待队列，接受等待队列，缓冲队列，通过加锁保证并发安全

无缓冲通道数据会直接在发送方和接收方复制传输
*/
